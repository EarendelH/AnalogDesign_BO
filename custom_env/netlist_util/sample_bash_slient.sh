...
# Loop through all subdirectories starting with 'tmp_'
foreach directory ("$target_directory"/tmp*/)
  if (-d "$directory") then
    ...
    # Process all .raw subdirectories
    foreach subdir ("$directory"/*.raw/)
      if (-d "$subdir") then
        foreach file ("$subdir"/*)
          if (-f "$file") then
            set start_psf = `date +%s`
            psf "$file" -o "${file}.encode" >& /dev/null
            set end_psf = `date +%s`
